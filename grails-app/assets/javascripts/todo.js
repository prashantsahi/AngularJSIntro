...
//end of selection
    ...
    $http.get('http://localhost:8080/api/group.json').success(function (data) {
        $scope.group = data;
    ...
    $scope.addTodoGroup = function (groupName) {
        alert(groupName);
        var dataGrp = {name: groupName, todos: []};

        $http.post('http://localhost:8080/api/group', dataGrp).success(function (data) {
            alert(data);
    ...
    $scope.removeTodoGroup = function (groupValue) {
        ...
        $http.delete('http://localhost:8080/api/group/' + groupValue.id + '.json').success(function () {
